...
s3_daily <- s3_daily %>% 
  ...
  mutate(
  menstruation_today = if_else(menstruation_since_last_entry == 1, menstruation_today, 0),
  ...
  mutate(
        ...
        RCD_inferred_squished = if_else(
          FCD > menstruation_length,
            NA_real_,
            as.numeric(if_else(
              menstruation_length - FCD < 14,
              29 - (menstruation_length - FCD),
              round((FCD/ (menstruation_length - 14) ) * 15))
            )),
...
# blake and juenger values are basically the same, so I'll use Juenger
